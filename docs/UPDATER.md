...
for deployment; the current coordinated profile requires Updater 0.4.6 or
...
- Updater 0.4.6 applies verified Compose files and missing safe environment defaults, preserving operator values and a complete deployment/env rollback snapshot; incompatible schema changes still require a qualified migration.
